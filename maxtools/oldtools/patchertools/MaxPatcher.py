...
class MaxPatcher(AbjadObject):
    ...
    def __call__(self, *args, **kwds):
        ...
        if len(args) == 1:
            ...
            self._attach_cue_items_to_context(context)
            self._attach_cues_to_context(context, previous_segment_metadata)
    ...
    def _attach_cue_items_to_context(self, context):
        ...
        # REDO this to put all cue items in the score and then attach cuesasspaners.
        from abjad import attach
        from abjad.tools import scoretools, durationtools
        import itertools
        cue_voice = self._get_cue_voice(context)
        ...
        for time_signature, group in itertools.groupby(timespans, lambda x: x.annotation.implied_time_signature):
            ...
            for timespan in group:
                timespan_start_offset, timespan_stop_offset = timespan.offsets
                offsets_with_cues_in_timespan = [ (start_offset, cues) for start_offset, cues in self._cue_items_by_start_offset.iteritems()
                    if timespan_start_offset <= start_offset < timespan_stop_offset
                    ]
                if not offsets_with_cues_in_timespan:
                    containers.append([])
        #         cues_in_timespan = [cue for cue in self._cues if timespan_start_offset <= cue.start_offset < timespan_stop_offset]
        #         if not cues_in_timespan:
        #             containers.append([])
                    ...
                    contents = []
                    if offsets_with_cues_in_timespan[0][0] > timespan_start_offset:
                        duration = offsets_with_cues_in_timespan[0][0] - timespan_start_offset
        #             if cues_in_timespan[0].start_offset > timespan_start_offset:
        #                 duration = cues_in_timespan[0].start_offset - timespan_start_offset
                        multiplier, duration = duration.yield_equivalent_durations()[0]
                        skip = scoretools.Skip(duration)
                        attach(multiplier, skip)
                        contents.append(skip)
                    for index, (start_offset, cues) in enumerate(offsets_with_cues_in_timespan):
                        print('{} {}'.format(index, start_offset))
                        if index == (len(offsets_with_cues_in_timespan) - 1):
                            duration = timespan_stop_offset - start_offset
                        else:
                            duration = offsets_with_cues_in_timespan[index + 1][0] - start_offset
        #             for index, cue in enumerate(cues_in_timespan):
        #                 if index == (len(cues_in_timespan) - 1):
        #                     duration = timespan_stop_offset - cue.start_offset
        #                 else:
    ...
    def _attach_cues_to_context(self, context, previous_segment_metadata):
        ...
        cue_indexes = []
        cues = []
        for index, note in enumerate(iterate(context).by_class(scoretools.Note).flatten()):
            cue_items = inspect_(note).get_indicators(prototype=cuetools.CueItem)
            if not all(cue_item.automatic for cue_item in cue_items):
                cue_indexes.append(index)
            if not cues:
                cue_indexes.append(index)
        for i, index in enumerate(cue_indexes):
            if i == len(cue_indexes) - 1:
                end_index = -1
            else:
                end_index = cue_indexes[i + 1]
            cue = cuetools.CueAsSpanner(number=current_cue_number)
            attach(cue, cue_voice[index:end_index])
            cues.append(cue)
